...
const Home = () => {
  ...
  useEffect(() => {
    async function ensureMonocleConnected() {
      await ensureConnected(logger, relayCallback);
      setConnected(true);
      await displayTextOnMonocle();
    }

    ensureMonocleConnected();
  }, []);

  async function displayTextOnMonocle() {
    await replRawMode(true);
    await replSend(`import display; display.text("VERBUNDEN", 0, 0, 0xffffff); display.show();`);
  }
